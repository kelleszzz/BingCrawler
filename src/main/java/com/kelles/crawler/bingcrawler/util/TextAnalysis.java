...
//		 pipeline.addAnnotator(new ParserAnnotator(false, -1));
	...
	protected static class SimHash {
		...
		public static String getBinaryStr(BigInteger t){return getBinaryStr(t,64);}
		...
		private BigInteger hash(String source,String charset) {
			if (source == null || source.length() == 0) {
				...
				byte[] byteArray=null;
				try {byteArray=source.getBytes(charset);} catch (UnsupportedEncodingException e) {e.printStackTrace();} 
				char[] sourceArray=new char[byteArray.length];
				for (int i=0;i<byteArray.length;i++) sourceArray[i]=(char)byteArray[i];
